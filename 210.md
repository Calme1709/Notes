...
##### Example:
.BLKW x4 - Allocates the next four words.
#### .FILL
...
# I/O
### I/O Controller
An IO Controller is a piece of hardware that is used to control an input or output device.

##### Registers
###### Control Register
A CPU can write to a control register to control the device.

###### Status Register
A CPU can read from a status register to see the current status of the device.

### Memory Mapped & Instruction Based IO
##### Instruction Based
There can be an IO instruction where we specify the device and operation i.e. IO Monitor Write and write that information to the devices registers.

##### Memory Mapped
We can assign some memory to an IO device and treat that as the registers of that device.

### Transfer control
##### Polling
The CPU will continuously check the status register of the device to check if anything needs to be done.

##### Interrupts
The device will send a CPU interrupt which will interrupt the CPU and let it know when something needs to be done.
